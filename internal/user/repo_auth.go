...
type AuthInterface interface {
	Set(*user_model.AuthInfo) (string, error)
	Get(string) (*user_model.AuthInfo, error)
	Del(string) error
}
...
// Set 生成 token
func (r *TokenAuth) Set(auth *user_model.AuthInfo) (string, error) {
	// 对称加密 auth
...
func (r *TokenAuth) Get(token string) (*user_model.AuthInfo, error) {
	// 解密 token
...
func (r *RedisAuth) Set(auth *user_model.AuthInfo) (string, error) {
	key := encryptKeyPrefix + auth.UserID
	status := r.c.Set(context.Background(), key, auth, r.expireTime)
	return key, status.Err()
}

func (r *RedisAuth) Get(token string) (*user_model.AuthInfo, error) {
	auth := &user_model.AuthInfo{}
